...
const Display = ({ cardState, handleClick }: CardProps) => {
  const _handleClick = () => {
    handleClick(cardState, cardState.id);
  };
  ...
  /* const conditionalValue = () => {
      ...
      if (timeGapAbsolute < 3 * 30)
        return { amount: Math.trunc(timeGapAbsolute / 7), scale: 'weeks' };
      if (timeGapAbsolute < 24 * 30)
        return { amount: Math.trunc(timeGapAbsolute / 30), scale: 'months' };
      // defaults to years
      ...
      return {
        amount: Math.trunc(timeGapAbsolute / 365),
        scale: 'years',
      };
    }; */
  return (
    ...
    >
      ...
      <div className="card-description">
        <span className="icon" role="img" aria-label="chat emoji">
          {' '}
          💬{' '}
        </span>
        <span className="title">{cardState.title}</span>
        {/*<span className="icon" role="img" aria-label="calendar emoji">*/}
        {/*  {' '}*/}
        {/*  📅{' '}*/}
        {/*</span>*/}
